...
import { poolManager } from '../dist/performance/ObjectPool.js';
import fs from 'fs';
...
async function runGCComparisonTest() {
  ...
  const gcReduction = withoutPooling.gcPressure > 0 
    ? ((withoutPooling.gcPressure - withPooling.gcPressure) / withoutPooling.gcPressure) * 100
    : 0;
  const timeReduction = withoutPooling.time > 0
    ? ((withoutPooling.time - withPooling.time) / withoutPooling.time) * 100
    : 0;
  ...
  fs.writeFileSync('./gc-pooling-results.json', JSON.stringify(results, null, 2));
